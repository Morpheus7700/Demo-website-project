...
const InternationalHolidaysPage = () => {
    ...
    useEffect(() => {
        const fetchHolidays = async () => {
            try {
                const res = await fetch('/api/trips/international');
                const data = await res.json();
                setHolidays(data);
            } catch (error) {
                console.error('Failed to fetch international holidays:', error);
            }
        };
        ...
        <div className="bg-white animate-fadeIn">
            <main className="container mx-auto px-6 py-16">
                <div className="text-center">
                    <h1 className="text-4xl md:text-6xl font-extrabold text-primary">International Holidays</h1>
                    <p className="text-lg md:text-xl text-gray-600 mt-4 max-w-3xl mx-auto">
                        Discover the world with our exciting international holiday packages.
                    </p>
                </div>
                ...
                <div className="mt-16 grid md:grid-cols-2 lg:grid-cols-3 gap-12">
                    {holidays.length > 0 ? (
                        ...
                        <p className="text-center md:col-span-3">No international holidays found.</p>
                    )}
                </div>
            </main>
        </div>
